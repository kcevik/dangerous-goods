    ...
    <div class="lg:grid lg:grid-cols-2 lg:gap-16 lg:items-center">
      ...
      <div class="rounded-2xl overflow-hidden border border-slate-200 shadow-xl bg-white">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-xs" aria-label="Regelwerksvergleich Beispiel UN 1203">
            <thead>
            ...
            <tbody class="divide-y divide-slate-100">
              {#each tableRows as row (row.name)}
                <tr class="hover:bg-slate-50 transition-colors">
                  ...
                  <td class="px-4 py-3">
                    {#if row.vpGrDeviation}
                      <span class="px-1.5 py-0.5 rounded text-xs font-medium" style="background: rgba(249,115,22,0.12); color: #ea580c;">{row.vpGr}</span>
                    {:else}
                      <span class="text-slate-600">{row.vpGr}</span>
                    {/if}
                  </td>
                  <td class="px-4 py-3">
                    {#if row.lqDeviation}
                      <span class="px-1.5 py-0.5 rounded text-xs font-medium" style="background: rgba(249,115,22,0.12); color: #ea580c;">{row.lq}</span>
                    {:else}
                      <span class="text-slate-600">{row.lq}</span>
                    {/if}
                  </td>
